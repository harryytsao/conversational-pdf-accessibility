...
function detectTable(items) {
  ...
  // Filter rows to find ones that have 2+ substantial items (likely table columns)
  // Ignore bullet points when counting cells
  const bulletChars = ["•", "◦", "▪", "■", "○", "●", "-", "*", "°"];
  const tableRows = rows.filter((row) => {
    const substantialCells = row.filter(
      (item) =>
        item.text.trim().length > 3 && !bulletChars.includes(item.text.trim())
    );
    return substantialCells.length >= 2 && substantialCells.length <= 8;
  });

  // Need at least 3 consistent table rows
